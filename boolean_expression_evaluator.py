...
def evaluate(tokens, document):
    ...
    def Z(i):
        if tokens[i] not in ['(', ')', 'and', 'or', 'not']:
            if tokens[i] in document: return i+1, True
            return i+1, False

        elif tokens[i] == '(':
            i, exp = E(i+1)
            if i == len(tokens): raise Exception(f"Expected token ')' at {i-1}. Instead got {tokens[i-1]}")
            if tokens[i] != ')': raise Exception(f"Expected token ')' at {i}. Instead got {tokens[i]}")
            return i+1, exp    
        
        else : raise Exception("Malformed Expresion")
